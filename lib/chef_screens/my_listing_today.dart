...
class MyListingToday extends StatelessWidget {
        ...
        child: StreamBuilder<List<FoodItem>>(
            ...
            builder: (context, snapshot) {
              ...
              for (int i = 0; i < items.length; i++) {
                print("i $i -- ${items[i].dishName}  " +
                    _firebaseMethodsChef
                        .tellMeDay(items[i].fromDate, items[i].toDate)
                        .toString());
                if ("today" ==
                    _firebaseMethodsChef.tellMeDay(
                        items[i].fromDate, items[i].toDate)) {
                  if (items[i].mealType.toLowerCase() == "breakfast")
                    breakfastItems.add(items[i]);
                  else if (items[i].mealType.toLowerCase() == "lunch")
